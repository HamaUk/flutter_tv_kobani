...
class _HomeState extends ConsumerState<Home> {
  ...
  // Category FocusNodes keyed by category name.
  // IMPORTANT: only add new entries, never replace existing ones.
  // Nodes that belong to categories that have disappeared are disposed in
  // _syncCategoryNodes().
  final Map<String, FocusNode> _categoryNodes = {};
  final FocusNode _liveTvFocusNode = FocusNode(debugLabel: 'LiveTV');
  final FocusNode _moviesFocusNode = FocusNode(debugLabel: 'Movies');
  final FocusNode _settingsFocusNode = FocusNode(debugLabel: 'Settings');
  bool _initialized = false;

  /// Call this every time the category list changes. It disposes nodes for
  /// categories that no longer exist and creates nodes for new ones – but
  /// never recreates a node for an existing category.
  void _syncCategoryNodes(List<String> currentCategories) {
    final currentSet = currentCategories.toSet();

    // Remove stale nodes
    final staleKeys = _categoryNodes.keys
        .where((k) => !currentSet.contains(k))
        .toList();
    for (final k in staleKeys) {
      _categoryNodes[k]!.dispose();
      _categoryNodes.remove(k);
    }

    // Add missing nodes
    for (final cat in currentCategories) {
      _categoryNodes.putIfAbsent(cat, () => FocusNode(debugLabel: 'Cat_$cat'));
    }
  }
        ...
        children: [
          ...
          channelsAsync.when(
            ...
            data: (allChannels) {
              ...
              // Keep category FocusNodes in sync – never recreates existing nodes
              _syncCategoryNodes(categories);

              if (_selectedCategory == null && categories.isNotEmpty) {
                // Auto-select first category on first data load
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  if (mounted) {
                    setState(() => _selectedCategory = categories.first);
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      if (mounted && _categoryNodes.isNotEmpty) {
        ...
        children: [
          ...
          _SidebarItem(
            ...
            onTap: () {
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted && _categoryNodes.isNotEmpty) _categoryNodes.values.first.requestFocus();
              });
            },
            onMoveRight: () {
              WidgetsBinding.instance.addPostFrameCallback((_) {
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _CategoryItem(
                  themeColor: themeColor,
                  focusNode: _categoryNodes[cat]!, // always exists after _syncCategoryNodes
                  title: cat,
